...
{{define "Attributes"}}
	{{range .}}
		...
		{{ else }}
			{{ .Name | makeFieldPublic}} string ` + "`" + `xml:"{{.Name}},attr,omitempty" cty:"{{ .Name | toSnakeCase }}"` + "`" + `
		{{ end }}
	{{end}}
{{end}}
		...
		{{if not .Type}}
			{{if .SimpleType}}
				...
				{{if ne .SimpleType.List.ItemType ""}}
					{{ .Name | makeFieldPublic}} []{{toGoType .SimpleType.List.ItemType}} ` + "`" + `xml:"{{.Name}},omitempty" cty:"{{ .Name | toSnakeCase }}"` + "`" + `
				{{else}}
